...
export default function TrustScreen({ userId, profile }: Props) {
  ...
  return (
    <div className="font-body" style={{ background: 'var(--color-bg)' }}>
      <div className="w-full">
        ...
        <div className="px-4 pb-4" style={{ paddingTop: 'calc(20px + var(--sat, 0px))' }}>
          <h1 className="font-heading text-[20px] font-light" style={{ color: 'var(--color-text)' }}>
            Trust <em style={{ color: 'var(--color-amber)' }}>Journey</em>
          </h1>
        ...
        {/* Trust level card */}
        <div className="mx-3 mb-3 rounded-[16px] p-4" style={{ background: 'var(--color-card)', border: '0.5px solid var(--color-amber-border)' }}>
          ...
          {nextLevel && (
            <div className="mt-3">
              <div className="flex justify-between mb-1">
                ...
                <span className="text-[11px]" style={{ color: 'var(--color-text-muted)' }}>{nextLevel.label}</span>
              </div>
              ...
              <div className="text-[11px] mt-1 text-center" style={{ color: 'var(--color-text-muted)' }}>
                {progressLabel}
              </div>
              {nextLevel.key === 'contributor' && (
                <div className="text-[10px] mt-0.5 text-center" style={{ color: 'var(--color-text-secondary)' }}>
          ...
          {!nextLevel && (
            <div className="text-[12px] text-center mt-1" style={{ color: 'var(--color-text-muted)' }}>
        ...
        <div className="flex gap-2 px-3 mb-4">
          {[
            ...
            <div key={stat.label} className="flex-1 rounded-[10px] px-2.5 py-2.5 text-center" style={{ background: 'var(--color-card)' }}>
              <div className="text-[16px] font-semibold" style={{ color: 'var(--color-amber)' }}>{stat.val}</div>
              <div className="text-[12px] mt-0.5 font-medium" style={{ color: 'var(--color-text)' }}>{stat.label}</div>
              <div className="text-[9px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{stat.sub}</div>
            </div>
          ))}
        </div>

        {/* Trust level ladder */}
        <div className="px-4 pb-2 text-[11px] uppercase" style={{ color: 'var(--color-text-muted)', letterSpacing: '0.1em' }}>
          Trust levels
        </div>
        <div className="mx-3 mb-4 rounded-[14px] overflow-hidden" style={{ border: '0.5px solid var(--color-border)' }}>
          {TRUST_LEVELS.map((level, i) => {
            ...
            return (
              <div
                ...
                style={{
                  background: isCurrent ? 'var(--color-amber-bg)' : 'var(--color-card)',
                  borderBottom: i < TRUST_LEVELS.length - 1 ? '0.5px solid var(--color-pill-bg)' : 'none',
                }}
              >
                <div
                  className="w-6 h-6 rounded-full flex items-center justify-center text-[12px] shrink-0 mt-0.5"
                  style={{ background: isUnlocked ? `${level.color}25` : 'var(--color-pill-bg)', border: isCurrent ? `1.5px solid ${level.color}` : '1px solid transparent' }}
                >
                  {level.emoji}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-medium" style={{ color: isUnlocked ? level.color : 'var(--color-text-muted)' }}>
                    {level.label}
                  </div>
                  ...
                  {level.unlock && (
                    <div className="text-[10px] mt-0.5 italic" style={{ color: 'var(--color-text-secondary)' }}>
                      {level.unlock}
                    </div>
                  )}
                </div>
                {isCurrent && (
                  <span className="text-[8px] font-semibold uppercase px-1.5 py-0.5 rounded-full shrink-0 mt-0.5" style={{ background: 'var(--color-amber)', color: 'var(--color-pill-active-text)', letterSpacing: '0.04em' }}>
                    You are here
                  </span>
                )}
              </div>
        ...
        <div className="px-3 space-y-1.5 mb-4">
          {GROW_TIPS.map(tip => (
            <div key={tip.action} className="rounded-[10px] px-3 py-2.5" style={{ background: 'var(--color-card)' }}>
              <div className="text-[13px] font-medium" style={{ color: 'var(--color-text)' }}>{tip.action}</div>
              <div className="text-[11px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>{tip.desc}</div>
            </div>
          ))}
        </div>
        ...
        <div className="px-3 pb-24">
          {endorsements.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {endorsements.map(e => (
                <div key={e.id} className="rounded-[10px] px-3 py-2.5" style={{ background: 'var(--color-card)', border: '0.5px solid var(--color-border)' }}>
                  <div className="flex items-center gap-2">
                    <div
                      className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-semibold shrink-0"
                      style={{ background: 'var(--color-avatar-bg)', border: '1px solid #2D5A1E', color: '#4A7C59' }}
                    >
                      {e.from_name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] font-medium" style={{ color: 'var(--color-text)' }}>{e.from_name}</div>
                      {e.message && (
                    ...
                    <div className="text-[9px] shrink-0" style={{ color: 'var(--color-text-muted)' }}>
