/** Biovilla — biovilla.org — Sustainability center near Arrábida (Palmela/Setúbal, Portugal). */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://biovilla.org/eventos', 'https://biovilla.org/events', 'https://www.biovilla.org/agenda']
const SRC = 'biovilla-pt', ORG = 'Biovilla', DEF_LAT = 38.5, DEF_LNG = -8.8
...
export const biovillaPt: SourceFetcher = {
  name: SRC,
  async fetch() {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                ...
                location_name: e.venue?.venue ?? e.venue?.city ?? 'Palmela, Portugal',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|experiencia|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      ...
      description: 'Evento Biovilla. Ver link para detalhes.', organizer: ORG,
      location_name: 'Palmela, Portugal', lat: DEF_LAT, lng: DEF_LNG,
