/**
 * MIRAMAP / Réseau AMAP — miramap.org / reseau-amap.org
 * French community-supported agriculture network events.
 */
...
const URLS = [
  'https://miramap.org/agenda/',
  'https://miramap.org/evenements/',
  'https://www.reseau-amap.org/agenda/',
]
  ...
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'amap-fr', source_id: `amap-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'MIRAMAP / Réseau AMAP',
        ...
        const jsonLd = extractJsonLd(html, 'amap-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[amap-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|agenda|amap|ferme|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'amap-fr', source_id: `amap-${hashStr(title)}`,
      source_url: link, title, description: `AMAP network event. See ${link}`,
      organizer: 'MIRAMAP / Réseau AMAP', location_name: locName, ...geo,
