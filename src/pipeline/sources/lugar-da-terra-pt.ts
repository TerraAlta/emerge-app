/** Quinta Lugar da Terra — retreat and workshop centre in central Portugal. */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://lugardaterra.pt/eventos', 'https://lugardaterra.pt/events', 'https://www.lugardaterra.pt/eventos']
const SRC = 'lugar-da-terra-pt'
const ORG = 'Quinta Lugar da Terra'
...
export const lugarDaTerraPt: SourceFetcher = {
  name: SRC,
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `ldt-${e.id}`, source_url: e.url ?? url,
                title: stripHtml(e.title ?? ''), description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: e.organizer?.[0]?.organizer ?? ORG,
                location_name: e.venue?.venue ?? 'Quinta Lugar da Terra, Portugal',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|retiro|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `ldt-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento Lugar da Terra. Ver link para detalhes.', organizer: ORG,
      location_name: 'Quinta Lugar da Terra, Portugal',
