/** Aldeias do Xisto — aldeiasdoxisto.pt — 27 schist villages, heritage tourism (central Portugal). */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://aldeiasdoxisto.pt/eventos', 'https://aldeiasdoxisto.pt/events', 'https://www.aldeiasdoxisto.pt/agenda']
const SRC = 'aldeias-xisto-pt', ORG = 'Aldeias do Xisto', DEF_LAT = 40.0, DEF_LNG = -7.8
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                ...
                location_name: e.venue?.venue ?? e.venue?.city ?? 'Centro de Portugal',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|festival|trilho|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      ...
      description: 'Evento Aldeias do Xisto. Ver link para detalhes.', organizer: ORG,
      location_name: 'Centro de Portugal', lat: DEF_LAT, lng: DEF_LNG,
