/**
 * Amsterdam Donut Coalitie — amsterdamdonutcoalitie.nl
 * Amsterdam's official Doughnut Economics coalition events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://amsterdamdonutcoalitie.nl/agenda',
  'https://amsterdamdonutcoalitie.nl/events',
  'https://amsterdamdonutcoalitie.nl/evenementen',
]
const ORG = 'Amsterdam Donut Coalitie'
const SRC = 'donut-amsterdam-nl'
...
export const donutAmsterdamNl: SourceFetcher = {
  name: SRC,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evenement|donut|bijeenkomst|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `dn-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: `${ORG} bijeenkomst. Zie link voor details.`,
      organizer: ORG, location_name: 'Amsterdam',
