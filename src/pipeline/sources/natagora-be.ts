 ...
 * Walloon nature conservation org, 10,000+ volunteers, French-speaking.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://www.natagora.be'
const URLS = [`${BASE}/agenda`, `${BASE}/activites`]
  ...
  async fetch() {
    for (const url of URLS) {
      try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: 'natagora-be', source_id: `nat-be-${e.id}`,
                source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: e.organizer?.[0]?.organizer ?? 'Natagora',
        ...
        const jsonLd = extractJsonLd(html, 'natagora-be')
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|activite|sortie|observation|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    const locName = loc ? stripHtml(loc[1]).trim() : 'Wallonie'
    events.push({
      source: 'natagora-be', source_id: `nat-be-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: 'Activité Natagora. Voir le lien pour détails.',
      organizer: 'Natagora', location_name: locName,
