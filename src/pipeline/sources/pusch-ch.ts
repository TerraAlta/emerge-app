/** Pusch — pusch.ch — Swiss environmental education and zero waste events */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://www.pusch.ch/veranstaltungen/', 'https://www.pusch.ch/events/', 'https://www.pusch.ch/agenda/']
...
export const puschCh: SourceFetcher = {
  ...
  async fetch() {
    try {
      const r = await fetch('https://www.pusch.ch/wp-json/tribe/events/v1/events?per_page=30', {
        headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(10000),
      })
      if (r.ok) {
        const d = await r.json()
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'pusch-ch', source_id: `pusch-t${e.id}`,
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'pusch-ch')
        if (ld.length > 0) return ld
        const out: RawEvent[] = []
        const rx = /<(?:article|div|li)[^>]*class="[^"]*(?:event|veranstaltung|kurs|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
        let m
        while ((m = rx.exec(html)) !== null) {
          ...
          out.push({
            source: 'pusch-ch', source_id: `pusch-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], url).toString() : url, title,
            description: 'Pusch environmental event.',
            organizer: 'Pusch', location_name: 'Switzerland',
