 ...
 * Farmland access and protection — community land trust model for farms.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://agrariantrust.org/events/',
  'https://agrariantrust.org/whats-happening/',
]

export const agrarianTrustUsa: SourceFetcher = {
  name: 'agrarian-trust-usa',
  async fetch(opts: { lat: number; lng: number; radiusKm: number }) {
    ...
    for (const url of URLS) {
      try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              for (const e of data.events) {
                allEvents.push({
                  source: 'agrarian-trust-usa', source_id: `at-${e.id}`,
                  source_url: e.url ?? url,
                  title: stripHtml(e.title ?? ''),
                  description: stripHtml(e.description ?? '').slice(0, 500),
                  organizer: 'Agrarian Trust',
        ...
        // Tier 2 — JSON-LD
        const jsonLd = extractJsonLd(html, 'agrarian-trust-usa')
        if (jsonLd.length > 0) { allEvents.push(...jsonLd); continue }

        // Tier 3 — HTML scrape fallback
        const pattern = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|farmland|trust|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
        let match
        while ((match = pattern.exec(html)) !== null) {
          ...
          allEvents.push({
            source: 'agrarian-trust-usa', source_id: `at-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], host).toString() : url,
            title,
            description: descMatch ? stripHtml(descMatch[1]).trim().slice(0, 500) : 'Agrarian Trust event — farmland access, community land trusts, farm protection.',
            organizer: 'Agrarian Trust',
        ...
        console.warn(`[agrarian-trust-usa] ${url} failed:`, (err as Error).message)
      }
    }
    return allEvents
  },
