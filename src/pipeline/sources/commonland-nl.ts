 ...
 * Landscape restoration foundation, 4 Returns framework.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://www.commonland.com/events/',
  'https://www.commonland.com/news-events/',
]
const ORG = 'Commonland'
const SRC = 'commonland-nl'
...
export const commonlandNl: SourceFetcher = {
  name: SRC,
        ...
        try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length) return data.events.map((e: any) => ({
              source: SRC, source_id: `cl-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|news|landscape|restoration|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `cl-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: `${ORG} event. See link for details.`,
      organizer: ORG, location_name: 'Amsterdam',
      lat: DEF.lat, lng: DEF.lng, starts_at: parseNL(block), cost: 'See event',
    })
  }
  return events
}
