 ...
 * Germany's biggest environmental NGO, 660,000 members. Actions, events, campaigns.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://www.bund.net'
const URLS = [`${BASE}/mitmachen/`, `${BASE}/service/termine/`]
  ...
  async fetch(_opts: { lat: number; lng: number; radiusKm: number }) {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'bund-de')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: 'bund-de', source_id: `bund-${e.id}`,
                source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'BUND \u2014 Bund f\u00fcr Umwelt und Naturschutz',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|termin|aktion|mitmachen|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    if (!title || title.length < 5 || /^(menu|nav|search|cookie|datenschutz)/i.test(title)) continue
    const desc = block.match(/<p[^>]*>([\s\S]*?)<\/p>/i)
    events.push({
      source: 'bund-de', source_id: `bund-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: desc ? stripHtml(desc[1]).trim().slice(0, 500) : 'BUND Aktion oder Veranstaltung.',
      organizer: 'BUND \u2014 Bund f\u00fcr Umwelt und Naturschutz', location_name: 'Deutschland',
