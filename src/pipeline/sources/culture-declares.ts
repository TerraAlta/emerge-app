/**
 * Culture Declares Emergency — culturedeclares.com
 * Arts & culture climate emergency declarations, actions, residencies.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://www.culturedeclares.com/events',
  'https://www.culturedeclares.com/actions',
]
const HEADERS = { 'User-Agent': 'Mozilla/5.0 (compatible; Emerge-App/1.0)', Accept: 'text/html' }

export const cultureDeclares: SourceFetcher = {
  name: 'culture-declares',
  async fetch() {
    ...
    for (const url of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            for (const e of (data.events ?? [])) {
              ...
              allEvents.push({
                source: 'culture-declares', source_id: `cd-${e.id ?? hashStr(e.title)}`,
                source_url: e.url ?? url, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Culture Declares Emergency',
        ...
        const jsonLd = extractJsonLd(html, 'culture-declares')
        if (jsonLd.length > 0) { allEvents.push(...jsonLd); continue }

        // 3. HTML scrape fallback
        const pat = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|action|declaration|residency|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
        let m
        while ((m = pat.exec(html)) !== null) {
          ...
          allEvents.push({
            source: 'culture-declares', source_id: `cd-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], origin).toString() : url, title,
            description: descMatch ? stripHtml(descMatch[1]).trim().slice(0, 500) : 'Culture Declares Emergency event.',
            organizer: 'Culture Declares Emergency', location_name: 'Global',
            lat: 0, lng: 0, starts_at: startsAt, cost: 'See event page',
          })
        }
      } catch (err) {
        console.warn(`[culture-declares] ${url} failed:`, (err as Error).message)
      }
    }
    return allEvents
  },
