/**
 * GAIA Portugal — gaia.org.pt/eventos
 * Portuguese permaculture and ecovillage network events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://gaia.org.pt/eventos/', 'https://gaia.org.pt/events/']
const SRC = 'gaia-pt'
const ORG = 'GAIA Portugal'
...
export const gaiaPt: SourceFetcher = {
  name: SRC,
  async fetch() {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `gaia-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `gaia-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento GAIA Portugal. Ver link para detalhes.', organizer: ORG,
