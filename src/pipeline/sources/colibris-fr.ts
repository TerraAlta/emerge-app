/**
 * Colibris France — colibris-lemouvement.org/agenda
 * French regenerative movement events.
 */
...
const URLS = [
  'https://www.colibris-lemouvement.org/agenda',
  'https://www.colibris-lemouvement.org/magazine/agenda',
]
  ...
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'colibris-fr', source_id: `colibris-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Colibris', location_name: e.venue?.venue ?? 'France',
        ...
        const jsonLd = extractJsonLd(html, 'colibris-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[colibris-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|agenda|evenement|article|views-row)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'colibris-fr', source_id: `colibris-${hashStr(title)}`,
      source_url: link, title, description: `Colibris event. See ${link}`,
      organizer: 'Colibris', location_name: locName, ...geo,
