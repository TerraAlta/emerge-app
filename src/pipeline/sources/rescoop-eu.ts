/** REScoop.eu — renewable energy cooperative events */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://rescoop.eu'
const URLS = [`${BASE}/events`, `${BASE}/news-events`]
  ...
  async fetch() {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (posts.length > 0) return posts.map(p => ({
          source: 'rescoop-eu', source_id: `rescoop-${p.id}`,
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'rescoop-eu', source_id: `rescoop-t${e.id}`,
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'rescoop-eu')
...
function scrape(html: string, base: string): RawEvent[] {
  const out: RawEvent[] = []
  const rx = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|news|webinar|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
  let m
  while ((m = rx.exec(html)) !== null) {
    ...
    out.push({
      source: 'rescoop-eu', source_id: `rescoop-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], base).toString() : base, title,
      description: desc ? stripHtml(desc[1]).slice(0, 500) : 'REScoop.eu energy cooperative event.',
      organizer: 'REScoop.eu', location_name: 'Europe', lat: 0, lng: 0,
