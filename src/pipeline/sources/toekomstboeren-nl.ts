 ...
 * Dutch future farming network events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://toekomstboeren.nl/agenda/',
  'https://toekomstboeren.nl/evenementen/',
]
        ...
        try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length) return data.events.map((e: any) => ({
              source: SRC, source_id: `tb-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evenement|agenda|boer|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `tb-${hashStr(title)}`,
