/** The Orchard Project — theorchardproject.org.uk — community orchard events */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://www.theorchardproject.org.uk/events/', 'https://theorchardproject.org.uk/events/', 'https://www.theorchardproject.org.uk/whats-on/']

export const orchardProjectUk: SourceFetcher = {
  name: 'orchard-project-uk',
  async fetch() {
    try {
      const r = await fetch('https://www.theorchardproject.org.uk/wp-json/tribe/events/v1/events?per_page=30', {
        headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(10000),
      })
      if (r.ok) {
        const d = await r.json()
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'orchard-project-uk', source_id: `orch-t${e.id}`,
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'orchard-project-uk')
        ...
        while ((m = rx.exec(html)) !== null) {
          ...
          out.push({
            source: 'orchard-project-uk', source_id: `orch-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], url).toString() : url, title,
            description: 'Community orchard event.',
            organizer: 'The Orchard Project', location_name: 'UK',
