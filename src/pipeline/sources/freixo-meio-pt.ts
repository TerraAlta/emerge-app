/** Herdade do Freixo do Meio — organic/biodynamic farm, Montemor-o-Novo, Alentejo. */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://www.hfreixodomeio.pt/eventos', 'https://www.hfreixodomeio.pt/events', 'https://freixodomeio.pt/eventos']
const SRC = 'freixo-meio-pt'
const ORG = 'Herdade do Freixo do Meio'
const DEFAULT_LAT = 38.7500
const DEFAULT_LNG = -7.9167
...
export const freixoMeioPt: SourceFetcher = {
  name: SRC,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|visita|atividade|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `fm-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento Freixo do Meio. Ver link para detalhes.', organizer: ORG,
      location_name: 'Freixo do Meio, Montemor-o-Novo, Alentejo',
