/**
 * Monnaies Locales Complémentaires — monnaieslocales.fr
 * Local complementary currency network events.
 */
...
const URLS = [
  'https://monnaieslocales.fr/agenda/',
  'https://monnaieslocales.fr/evenements/',
]
const HEADERS = { 'User-Agent': 'Mozilla/5.0 (compatible; Emerge-App/1.0)', Accept: 'text/html' }

export const monnaiesLocalesFr: SourceFetcher = {
  name: 'monnaies-locales-fr',
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'monnaies-locales-fr', source_id: `monnaies-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Monnaies Locales Complémentaires',
        ...
        const jsonLd = extractJsonLd(html, 'monnaies-locales-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[monnaies-locales-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|agenda|monnaie|rencontre|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'monnaies-locales-fr', source_id: `monnaies-${hashStr(title)}`,
      source_url: link, title, description: `Local currency network event. See ${link}`,
      organizer: 'Monnaies Locales Complémentaires', location_name: locName, ...geo,
