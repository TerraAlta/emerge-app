 ...
 * agroecologia.net/eventos — organic agriculture events in Spain.
 */
...
const URLS = [
  'https://www.agroecologia.net/eventos/',
  'https://www.agroecologia.net/agenda/',
]
const SRC = 'seae-es'
const ORG = 'SEAE — Sociedad Española de Agricultura Ecológica'
const DEFAULT_LAT = 40.4168
const DEFAULT_LNG = -3.7038

export const seaeEs: SourceFetcher = {
  name: SRC,
  async fetch() {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `seae-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|jornada|congreso|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `seae-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento de agricultura ecológica. Ver enlace para detalles.', organizer: ORG,
