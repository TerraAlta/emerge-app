/** Cohousing Europe — events & open days */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://cohousing.eu'
  ...
  async fetch() {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'cohousing-eu', source_id: `cohous-t${e.id}`,
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'cohousing-eu')
...
function scrape(html: string, base: string): RawEvent[] {
  const out: RawEvent[] = []
  const rx = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|open-day|workshop|community|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
  let m
  while ((m = rx.exec(html)) !== null) {
    ...
    out.push({
      source: 'cohousing-eu', source_id: `cohous-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], base).toString() : base, title,
      description: desc ? stripHtml(desc[1]).slice(0, 500) : 'Cohousing Europe event.',
      organizer: 'Cohousing Europe', location_name: 'Europe', lat: 0, lng: 0,
