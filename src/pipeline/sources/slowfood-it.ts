/**
 * Slow Food Italia — slowfood.it events
 * Separate from slowfood-global.ts which covers international events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://www.slowfood.it/eventi/',
  'https://www.slowfood.it/cosa-facciamo/eventi/',
]
const DEFAULT_LAT = 44.6939
const DEFAULT_LNG = 8.0364
  ...
  async fetch(opts: { lat: number; lng: number; radiusKm: number }) {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'slowfood-it')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: 'slowfood-it', source_id: `sf-${e.id}`,
                source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: e.organizer?.[0]?.organizer ?? 'Slow Food Italia',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pattern = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|mercato|terra-madre|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let match
  while ((match = pattern.exec(html)) !== null) {
    ...
    events.push({
      source: 'slowfood-it', source_id: `sf-${hashStr(title)}`,
      source_url: titleMatch[1] ? new URL(titleMatch[1], baseUrl).toString() : baseUrl,
      title, description: 'Evento Slow Food Italia. Vedi link per dettagli.',
