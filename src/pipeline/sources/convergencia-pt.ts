/** Convergência de Permacultura — redeconvergir.net — Permaculture convergence (Portugal). */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://redeconvergir.net/convergencia', 'https://redeconvergir.net/convergencia-de-permacultura']
const SRC = 'convergencia-pt', ORG = 'Convergência de Permacultura', DEF_LAT = 38.7169, DEF_LNG = -9.1393
...
export const convergenciaPt: SourceFetcher = {
  name: SRC,
  async fetch() {
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|convergencia|permacultura|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      ...
      description: 'Evento de permacultura. Ver link para detalhes.', organizer: ORG,
      location_name: 'Portugal', lat: DEF_LAT, lng: DEF_LNG,
