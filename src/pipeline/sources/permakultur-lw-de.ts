/**
 * Permakultur Landwirtschaft — permakultur-landwirtschaft.org
 * Permaculture farming network. Workshops, farm tours, training.
 */
  ...
  async fetch() {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'permakultur-lw-de')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: 'permakultur-lw-de', source_id: `plw-${e.id}`,
                source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Permakultur Landwirtschaft',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|veranstaltung|kurs|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'permakultur-lw-de', source_id: `plw-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: desc ? stripHtml(desc[1]).trim().slice(0, 500) : 'Permakultur Landwirtschaft Veranstaltung.',
      organizer: 'Permakultur Landwirtschaft', location_name: 'Deutschland',
