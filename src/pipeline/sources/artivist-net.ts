/**
 * Artivist Network — artivistnetwork.org / artivist.com
 * Art activism events, projects, and gatherings.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://www.artivistnetwork.org/events',
  'https://www.artivist.com/events',
]
const HEADERS = { 'User-Agent': 'Mozilla/5.0 (compatible; Emerge-App/1.0)', Accept: 'text/html' }

export const artivistNet: SourceFetcher = {
  name: 'artivist-net',
  async fetch() {
    ...
    for (const url of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            for (const e of (data.events ?? [])) {
              ...
              allEvents.push({
                source: 'artivist-net', source_id: `an-${e.id ?? hashStr(e.title)}`,
                source_url: e.url ?? url, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Artivist Network',
        ...
        const jsonLd = extractJsonLd(html, 'artivist-net')
        if (jsonLd.length > 0) { allEvents.push(...jsonLd); continue }

        // 3. HTML scrape fallback
        const pat = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|project|action|gathering|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
        let m
        while ((m = pat.exec(html)) !== null) {
          ...
          allEvents.push({
            source: 'artivist-net', source_id: `an-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], origin).toString() : url, title,
            description: descMatch ? stripHtml(descMatch[1]).trim().slice(0, 500) : 'Artivist Network event.',
            organizer: 'Artivist Network', location_name: 'Global',
            lat: 0, lng: 0, starts_at: startsAt, cost: 'See event page',
          })
        }
      } catch (err) {
        console.warn(`[artivist-net] ${url} failed:`, (err as Error).message)
      }
    }
    return allEvents
  },
