/**
 * Red Española de Permacultura — permacultura.es/eventos
 * Spanish permaculture network events.
 */
...
const URLS = [
  'https://permacultura.es/eventos/',
  'https://www.permacultura-es.org/eventos',
]
const SRC = 'permacultura-es'
const ORG = 'Red Española de Permacultura'

export const permaculturaEs: SourceFetcher = {
  name: SRC,
  async fetch() {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `perm-es-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|curso|taller|encuentro|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `perm-es-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento de permacultura. Ver enlace para detalles.', organizer: ORG,
