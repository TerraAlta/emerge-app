/**
 * Red Ibérica de Ecoaldeas — ecoaldeas.es/eventos
 * Spanish ecovillage network events.
 */
...
const URLS = [
  'https://ecoaldeas.es/eventos/',
  'https://www.ecoaldeas.org/eventos/',
]
const SRC = 'ecoaldeas-es'
const ORG = 'Red Ibérica de Ecoaldeas'

export const ecoaldeasEs: SourceFetcher = {
  name: SRC,
  async fetch() {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `eco-es-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|ecoaldea|encuentro|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `eco-es-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento de ecoaldeas. Ver enlace para detalles.', organizer: ORG,
