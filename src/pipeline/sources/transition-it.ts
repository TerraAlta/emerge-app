/**
 * Transizione Italia — transizioneitalia.it
 * Italian transition movement events.
 */
...
const URLS = [
  'https://www.transizioneitalia.it/eventi/',
  'https://transizioneitalia.wordpress.com/eventi/',
]

export const transitionIt: SourceFetcher = {
  name: 'transition-it',
  async fetch() {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'transition-it')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => {
                ...
                return {
                  source: 'transition-it', source_id: `trans-it-${e.id}`,
                  source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                  description: stripHtml(e.description ?? '').slice(0, 500),
                  organizer: e.organizer?.[0]?.organizer ?? 'Transizione Italia',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pattern = /<(?:article|div|li)[^>]*class="[^"]*(?:event|iniziativa|attivita|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let match
  while ((match = pattern.exec(html)) !== null) {
    ...
    events.push({
      source: 'transition-it', source_id: `trans-it-${hashStr(title)}`,
      source_url: titleMatch[1] ? new URL(titleMatch[1], baseUrl).toString() : baseUrl,
      title, description: 'Evento di transizione. Vedi link per dettagli.',
      organizer: 'Transizione Italia', location_name: location,
