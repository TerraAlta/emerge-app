/**
 * Terre de Liens — terredeliens.org
 * French farmland access and community events.
 */
  ...
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'terredeliens-fr', source_id: `tdl-${hashStr(e.title + e.start_date)}`,
        ...
        const jsonLd = extractJsonLd(html, 'terredeliens-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[terredeliens-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|agenda|ferme|rencontre|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'terredeliens-fr', source_id: `tdl-${hashStr(title)}`,
      source_url: link, title, description: `Terre de Liens event. See ${link}`,
