/**
 * Agroecologie Netwerk — agroecologypartnership.eu
 * Dutch/EU agroecology network events and conferences.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://agroecologypartnership.eu/events/',
  'https://agroecologypartnership.eu/agenda/',
]
        ...
        try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length) return data.events.map((e: any) => ({
              source: SRC, source_id: `ae-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evenement|conference|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `ae-${hashStr(title)}`,
