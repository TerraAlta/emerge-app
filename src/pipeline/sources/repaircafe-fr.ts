/**
 * Repair Café France — repaircafe.org/fr
 * Community repair events across France.
 */
...
const URLS = [
  ...
  'https://www.repaircafe.org/fr/evenements/',
]
  ...
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'repaircafe-fr', source_id: `repaircafe-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Repair Café France', location_name: e.venue?.venue ?? 'France',
        ...
        const jsonLd = extractJsonLd(html, 'repaircafe-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[repaircafe-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|repair|cafe|atelier|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'repaircafe-fr', source_id: `repaircafe-${hashStr(title)}`,
      source_url: link, title, description: `Repair Café event. See ${link}`,
      organizer: 'Repair Café France', location_name: locName, ...geo,
