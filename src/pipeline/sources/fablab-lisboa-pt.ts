/** Fab Lab Lisboa — fablablisboa.pt — Maker/fabrication lab (Lisbon, Portugal). */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://fablablisboa.pt/eventos', 'https://fablablisboa.pt/events', 'https://www.fablablisboa.pt/agenda']
const SRC = 'fablab-lisboa-pt', ORG = 'Fab Lab Lisboa', DEF_LAT = 38.7169, DEF_LNG = -9.1393
...
export const fablabLisboaPt: SourceFetcher = {
  name: SRC,
  async fetch() {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                ...
                location_name: e.venue?.venue ?? e.venue?.city ?? 'Lisboa',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|workshop|maker|oficina|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      ...
      description: 'Evento Fab Lab Lisboa. Ver link para detalhes.', organizer: ORG,
