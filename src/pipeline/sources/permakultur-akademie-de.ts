/**
 * Permakultur Akademie — permakultur-akademie.de
 * Germany's main permaculture education body. PDC courses, seminars, events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://permakultur-akademie.de'
  ...
  async fetch(_opts: { lat: number; lng: number; radiusKm: number }) {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'permakultur-akademie-de')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: 'permakultur-akademie-de', source_id: `pka-${e.id}`,
                source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Permakultur Akademie',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|veranstaltung|kurs|seminar|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'permakultur-akademie-de', source_id: `pka-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: desc ? stripHtml(desc[1]).trim().slice(0, 500) : 'Permakultur Akademie Veranstaltung.',
      organizer: 'Permakultur Akademie', location_name: 'Deutschland',
