 ...
 * Italian ecovillage network events.
 */
...
const URLS = [
  'https://www.villaggiecologici.it/eventi/',
  'https://www.ecovillaggi.it/eventi/',
]
  ...
  async fetch() {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'rive-it')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => {
                ...
                return {
                  source: 'rive-it', source_id: `rive-${e.id}`,
                  source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                  description: stripHtml(e.description ?? '').slice(0, 500),
                  organizer: e.organizer?.[0]?.organizer ?? 'RIVE \u2014 Rete Italiana Villaggi Ecologici',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pattern = /<(?:article|div|li)[^>]*class="[^"]*(?:event|villaggio|incontro|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let match
  while ((match = pattern.exec(html)) !== null) {
    ...
    events.push({
      source: 'rive-it', source_id: `rive-${hashStr(title)}`,
      source_url: titleMatch[1] ? new URL(titleMatch[1], baseUrl).toString() : baseUrl,
      title, description: 'Evento ecovillaggio. Vedi link per dettagli.',
      organizer: 'RIVE \u2014 Rete Italiana Villaggi Ecologici', location_name: location,
