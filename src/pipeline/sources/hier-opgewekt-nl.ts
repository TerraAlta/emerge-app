/**
 * HIER opgewekt — hieropgewekt.nl
 * Community energy movement, 600+ cooperatives across the Netherlands.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://www.hieropgewekt.nl/evenementen',
  'https://www.hieropgewekt.nl/agenda',
]
const ORG = 'HIER opgewekt'
const SRC = 'hier-opgewekt-nl'
...
export const hierOpgewektNl: SourceFetcher = {
  name: SRC,
        ...
        try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length) return data.events.map((e: any) => ({
              source: SRC, source_id: `ho-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evenement|energie|lokaal|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `ho-${hashStr(title)}`,
