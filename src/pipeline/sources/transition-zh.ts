/**
 * Transition Zürich — transition-zuerich.ch
 * Local transition movement events in Zürich.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'
import { extractMultiDate, geocodeSwiss } from './ch-mt-utils'

const BASE = 'https://www.transition-zuerich.ch'
const URLS = [`${BASE}/agenda/`, `${BASE}/events/`]
const DEFAULT_LAT = 47.3769, DEFAULT_LNG = 8.5417

export const transitionZh: SourceFetcher = {
  name: 'transition-zh',
  async fetch() {
      ...
      if (apiRes.ok) {
        ...
        if (data.events?.length > 0) {
          return data.events.map((e: any) => {
            ...
            return {
              source: 'transition-zh', source_id: `tr-zh-${e.id}`,
              source_url: e.url ?? URLS[0], title: stripHtml(e.title ?? ''),
              description: stripHtml(e.description ?? '').slice(0, 500),
              organizer: e.organizer?.[0]?.organizer ?? 'Transition Zürich',
    ...
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'transition-zh')
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'transition-zh', source_id: `tr-zh-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Transition Zürich Veranstaltung.', organizer: 'Transition Zürich',
      location_name: geo ? 'Zürich' : 'Zürich',
