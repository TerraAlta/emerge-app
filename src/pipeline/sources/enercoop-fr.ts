/**
 * Enercoop — enercoop.fr
 * Renewable energy cooperative events, 100,000+ members.
 */
...
const URLS = [
  'https://www.enercoop.fr/nos-actualites',
  'https://www.enercoop.fr/evenements',
]
  ...
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'enercoop-fr', source_id: `enercoop-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Enercoop', location_name: e.venue?.venue ?? 'France',
        ...
        const jsonLd = extractJsonLd(html, 'enercoop-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[enercoop-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|actualite|energie|rencontre|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'enercoop-fr', source_id: `enercoop-${hashStr(title)}`,
      source_url: link, title, description: `Enercoop energy cooperative event. See ${link}`,
      organizer: 'Enercoop', location_name: locName, ...geo,
