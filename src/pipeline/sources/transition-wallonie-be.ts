/**
 * Réseau Transition Belgique — reseautransition.be
 * French-speaking Wallonia transition network events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'
import { extractFrenchDate, geocodeFrBe } from './french-utils'

const BASE = 'https://www.reseautransition.be'
const URLS = [
  `${BASE}/agenda/`,
  `${BASE}/evenements/`,
]

export const transitionWallonieBe: SourceFetcher = {
  name: 'transition-wallonie-be',
  async fetch() {
    for (const url of URLS) {
      try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => {
                ...
                return {
                  source: 'transition-wallonie-be', source_id: `tw-be-${e.id}`,
                  source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                  description: stripHtml(e.description ?? '').slice(0, 500),
                  organizer: e.organizer?.[0]?.organizer ?? 'Réseau Transition Belgique',
        ...
        const jsonLd = extractJsonLd(html, 'transition-wallonie-be')
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pattern = /<(?:article|div|li)[^>]*class="[^"]*(?:event|agenda|initiative|transition|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let match
  while ((match = pattern.exec(html)) !== null) {
    ...
    events.push({
      source: 'transition-wallonie-be', source_id: `tw-be-${hashStr(title)}`,
      source_url: titleMatch[1] ? new URL(titleMatch[1], baseUrl).toString() : baseUrl,
      title, description: 'Événement Réseau Transition. Voir le lien.',
      organizer: 'Réseau Transition Belgique', location_name: locName,
