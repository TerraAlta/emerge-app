/**
 * Friends of the Earth Malta — foemalta.org
 * Environmental campaigns and cleanup events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'
import { extractMultiDate, geocodeMalta } from './ch-mt-utils'
const URLS = [
  'https://foemalta.org/events/',
  'https://www.foemalta.org/events/',
]

export const foeMt: SourceFetcher = {
  name: 'foe-mt',
  async fetch() {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'foe-mt')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => {
                ...
                return {
                  source: 'foe-mt', source_id: `foe-mt-${e.id}`,
                  source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                  description: stripHtml(e.description ?? '').slice(0, 500),
                  organizer: e.organizer?.[0]?.organizer ?? 'Friends of the Earth Malta',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|activity|campaign|cleanup|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'foe-mt', source_id: `foe-mt-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: 'Environmental event. See link for details.',
      organizer: 'Friends of the Earth Malta', location_name: location,
