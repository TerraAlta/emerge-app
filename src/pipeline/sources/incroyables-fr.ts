/**
 * Incroyables Comestibles France — lesincroyablescomestibles.fr
 * Community edible gardening events.
 */
...
const URLS = [
  'https://lesincroyablescomestibles.fr/agenda/',
  'https://lesincroyablescomestibles.fr/evenements/',
]
const HEADERS = { 'User-Agent': 'Mozilla/5.0 (compatible; Emerge-App/1.0)', Accept: 'text/html' }

export const incroyablesFr: SourceFetcher = {
  name: 'incroyables-fr',
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'incroyables-fr', source_id: `incroyables-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Incroyables Comestibles France',
        ...
        const jsonLd = extractJsonLd(html, 'incroyables-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[incroyables-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|agenda|jardin|comestible|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'incroyables-fr', source_id: `incroyables-${hashStr(title)}`,
      source_url: link, title, description: `Incroyables Comestibles event. See ${link}`,
      organizer: 'Incroyables Comestibles France', location_name: locName, ...geo,
