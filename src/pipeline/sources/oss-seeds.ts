/** Open Source Seeds Initiative — events & seed festivals */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://opensourceseeds.org'
const URLS = [`${BASE}/en/events`, `${BASE}/veranstaltungen`]

export const ossSeeds: SourceFetcher = {
  name: 'oss-seeds',
  async fetch() {
    // 1. Try WP REST API
    try {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'oss-seeds', source_id: `osseeds-t${e.id}`,
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'oss-seeds')
...
function scrape(html: string, base: string): RawEvent[] {
  const out: RawEvent[] = []
  const rx = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|seed|festival|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
  let m
  while ((m = rx.exec(html)) !== null) {
    ...
    out.push({
      source: 'oss-seeds', source_id: `osseeds-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], base).toString() : base, title,
      description: desc ? stripHtml(desc[1]).slice(0, 500) : 'Open Source Seeds event.',
      organizer: 'Open Source Seeds Initiative', location_name: 'Europe', lat: 0, lng: 0,
