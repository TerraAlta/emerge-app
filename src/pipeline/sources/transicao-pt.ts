/** Portugal em Transição — transicao.org — Transition network (Portugal). */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://transicao.org/eventos', 'https://transicao.org/events', 'https://www.transicao.org/agenda']
const SRC = 'transicao-pt', ORG = 'Portugal em Transição', DEF_LAT = 38.7169, DEF_LNG = -9.1393
...
export const transicaoPt: SourceFetcher = {
  name: SRC,
  async fetch() {
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|iniciativa|transicao|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      ...
      description: 'Evento de transição. Ver link para detalhes.', organizer: ORG,
