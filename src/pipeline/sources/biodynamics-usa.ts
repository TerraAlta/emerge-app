 ...
 * Conferences, workshops, and farm events on biodynamic agriculture.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://www.biodynamics.com/events',
  'https://www.biodynamics.com/calendar',
]

export const biodynamicsUsa: SourceFetcher = {
  name: 'biodynamics-usa',
  async fetch() {
    ...
    for (const url of URLS) {
      try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              for (const e of data.events) {
                allEvents.push({
                  source: 'biodynamics-usa', source_id: `bda-${e.id}`,
                  source_url: e.url ?? url,
                  title: stripHtml(e.title ?? ''),
                  description: stripHtml(e.description ?? '').slice(0, 500),
                  organizer: 'Biodynamic Association',
        ...
        // Tier 2 — JSON-LD
        const jsonLd = extractJsonLd(html, 'biodynamics-usa')
        if (jsonLd.length > 0) { allEvents.push(...jsonLd); continue }

        // Tier 3 — HTML scrape fallback
        const pattern = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|conference|workshop|farm|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
        let match
        while ((match = pattern.exec(html)) !== null) {
          ...
          allEvents.push({
            source: 'biodynamics-usa', source_id: `bda-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], host).toString() : url,
            title,
            description: descMatch ? stripHtml(descMatch[1]).trim().slice(0, 500) : 'Biodynamic agriculture event — farming, soil health, cosmic rhythms.',
            organizer: 'Biodynamic Association',
        ...
        console.warn(`[biodynamics-usa] ${url} failed:`, (err as Error).message)
      }
    }
    return allEvents
  },
