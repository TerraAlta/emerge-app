/**
 * Les Compaillons — compaillons.org
 * Straw bale building network — community construction events.
 */
...
const URLS = [
  'https://www.compaillons.org/agenda/',
  'https://www.compaillons.org/evenements/',
]
  ...
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'compaillons-fr', source_id: `compaillons-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Les Compaillons', location_name: e.venue?.venue ?? 'France',
        ...
        const jsonLd = extractJsonLd(html, 'compaillons-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[compaillons-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|chantier|construction|paille|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'compaillons-fr', source_id: `compaillons-${hashStr(title)}`,
      source_url: link, title, description: `Straw bale construction event. See ${link}`,
      organizer: 'Les Compaillons', location_name: locName, ...geo,
