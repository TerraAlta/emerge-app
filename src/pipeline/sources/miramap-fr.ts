/**
 * MIRAMAP — miramap.org
 * National AMAP (community-supported agriculture) network events.
 */
...
const URLS = [
  'https://miramap.org/Agenda.html',
  'https://miramap.org/spip.php?rubrique3',
]
  ...
  async fetch() {
    for (const base of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            return evts.filter((e: any) => e.title && e.start_date).map((e: any): RawEvent => {
              ...
              return {
                source: 'miramap-fr', source_id: `miramap-${hashStr(e.title + e.start_date)}`,
                source_url: e.url ?? base, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'MIRAMAP', location_name: e.venue?.venue ?? 'France',
        ...
        const jsonLd = extractJsonLd(html, 'miramap-fr')
        if (jsonLd.length > 0) return jsonLd
        return scrapeHtml(html, base)
      } catch { continue }
    }
    console.warn('[miramap-fr] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|agenda|amap|rencontre|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'miramap-fr', source_id: `miramap-${hashStr(title)}`,
      source_url: link, title, description: `MIRAMAP AMAP network event. See ${link}`,
      organizer: 'MIRAMAP', location_name: locName, ...geo,
