/** Common Ground Apple Day — commonground.org.uk — annual October 21 celebration */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://www.commonground.org.uk/apple-day/', 'https://commonground.org.uk/apple-day/', 'https://www.commonground.org.uk/events/']

export const appleDayUk: SourceFetcher = {
  name: 'apple-day-uk',
  async fetch() {
    try {
      const r = await fetch('https://www.commonground.org.uk/wp-json/tribe/events/v1/events?per_page=30', {
        headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(10000),
      })
      if (r.ok) {
        const d = await r.json()
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'apple-day-uk', source_id: `aday-t${e.id}`,
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'apple-day-uk')
        if (ld.length > 0) return ld
        const out: RawEvent[] = []
        const rx = /<(?:article|div|li)[^>]*class="[^"]*(?:event|apple|festival|celebration|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
        let m
        while ((m = rx.exec(html)) !== null) {
          ...
          out.push({
            source: 'apple-day-uk', source_id: `aday-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], url).toString() : url, title,
            description: 'Apple Day community celebration.',
            organizer: 'Common Ground', location_name: 'UK',
