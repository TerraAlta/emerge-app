/**
 * PINA — Pender Island Natural Area
 * Pender Island, BC
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://pina.in'
const URLS = [`${BASE}/calendar/`, `${BASE}/events/`]
const DEFAULT_LAT = 48.77
const DEFAULT_LNG = -123.27
  ...
  async fetch(opts: { lat: number; lng: number; radiusKm: number }) {
      ...
      if (res.ok) {
        ...
        if (data.events?.length > 0) {
          return data.events.map((e: any) => ({
            ...
            organizer: e.organizer?.[0]?.organizer ?? 'PINA \u2014 Pender Island Natural Area',
            location_name: e.venue?.venue ?? 'Pender Island, BC',
    ...
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'pina-ca')
...
function scrape(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|calendar|workshop|nature|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'pina-ca', source_id: `pina-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: 'PINA event. See link for details.',
      organizer: 'PINA \u2014 Pender Island Natural Area',
      location_name: 'Pender Island, BC',
