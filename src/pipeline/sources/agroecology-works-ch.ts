/** Agroecology Works — agroecologyworks.ch — Swiss agroecology events */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://agroecologyworks.ch/events/', 'https://agroecologyworks.ch/veranstaltungen/', 'https://agroecologyworks.ch/agenda/']
...
export const agroecologyWorksCh: SourceFetcher = {
  name: 'agroecology-works-ch',
  async fetch() {
    try {
      const r = await fetch('https://agroecologyworks.ch/wp-json/tribe/events/v1/events?per_page=30', {
        headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(10000),
      })
      if (r.ok) {
        const d = await r.json()
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'agroecology-works-ch', source_id: `awch-t${e.id}`,
          source_url: e.url ?? null, title: stripHtml(e.title ?? ''),
          description: stripHtml(e.description ?? '').slice(0, 500),
          organizer: 'Agroecology Works',
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'agroecology-works-ch')
        ...
        while ((m = rx.exec(html)) !== null) {
          ...
          out.push({
            source: 'agroecology-works-ch', source_id: `awch-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], url).toString() : url, title,
            description: 'Agroecology event in Switzerland.',
            organizer: 'Agroecology Works', location_name: 'Switzerland',
