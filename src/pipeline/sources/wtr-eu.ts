 ...
 * Joanna Macy's spiral, grief & resilience workshops across Europe.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = [
  'https://workthatreconnects.org/events',
  'https://workthatreconnects.org/workshops',
]
const HEADERS = { 'User-Agent': 'Mozilla/5.0 (compatible; Emerge-App/1.0)', Accept: 'text/html' }

export const wtrEu: SourceFetcher = {
  name: 'wtr-eu',
  async fetch() {
    ...
    for (const url of URLS) {
      try {
        ...
        try {
          ...
          if (api.ok) {
            ...
            for (const e of (data.events ?? [])) {
              ...
              allEvents.push({
                source: 'wtr-eu', source_id: `wtr-${e.id ?? hashStr(e.title)}`,
                source_url: e.url ?? url, title: stripHtml(e.title),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: 'Work That Reconnects',
        ...
        const jsonLd = extractJsonLd(html, 'wtr-eu')
        if (jsonLd.length > 0) { allEvents.push(...jsonLd); continue }

        // 3. HTML scrape fallback
        const pat = /<(?:article|div|li|section)[^>]*class="[^"]*(?:event|workshop|spiral|gathering|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li|section)>/gi
        let m
        while ((m = pat.exec(html)) !== null) {
          ...
          allEvents.push({
            source: 'wtr-eu', source_id: `wtr-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], origin).toString() : url, title,
            description: descMatch ? stripHtml(descMatch[1]).trim().slice(0, 500) : 'Work That Reconnects workshop.',
            organizer: 'Work That Reconnects', location_name: 'Global',
            lat: 0, lng: 0, starts_at: startsAt, cost: 'See event page',
          })
        }
      } catch (err) {
        console.warn(`[wtr-eu] ${url} failed:`, (err as Error).message)
      }
    }
    return allEvents
  },
