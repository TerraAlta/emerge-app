/** Rede REGENERAR — regenerar.pt — Regeneration network (Portugal). */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://regenerar.pt/eventos', 'https://regenerar.pt/events', 'https://www.regenerar.pt/agenda']
const SRC = 'regenerar-pt', ORG = 'Rede REGENERAR', DEF_LAT = 38.7169, DEF_LNG = -9.1393
...
export const regenerarPt: SourceFetcher = {
  name: SRC,
  async fetch() {
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|regeneracao|formacao|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      ...
      description: 'Evento REGENERAR. Ver link para detalhes.', organizer: ORG,
