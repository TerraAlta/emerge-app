/**
 * Agricultura Regenerativa — agriculturaregenerativa.es + regeneraiberia.com
 * Regenerative agriculture courses, events and training in Spain.
 */
...
const URLS = [
  ...
  'https://www.agriculturaregenerativa.es/eventos/',
  'https://regeneraiberia.com/eventos/',
]
const SRC = 'agri-regen-es'
const ORG = 'Agricultura Regenerativa'
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `aregen-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evento|formacion|curso|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `aregen-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento de agricultura regenerativa. Ver enlace para detalles.', organizer: ORG,
