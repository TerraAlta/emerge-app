/** Kuringa Berlin — kuringa.net — Theatre of the Oppressed events in Berlin */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const URLS = ['https://kuringa.net/veranstaltungen/', 'https://kuringa.net/events/', 'https://www.kuringa.net/veranstaltungen/']
...
export const kuringaDe: SourceFetcher = {
  name: 'kuringa-de',
  async fetch() {
    try {
      const r = await fetch('https://kuringa.net/wp-json/tribe/events/v1/events?per_page=30', {
        headers: { Accept: 'application/json' }, signal: AbortSignal.timeout(10000),
      })
      if (r.ok) {
        const d = await r.json()
        if (d.events?.length) return d.events.map((e: any) => ({
          source: 'kuringa-de', source_id: `kur-t${e.id}`,
          source_url: e.url ?? null, title: stripHtml(e.title ?? ''),
          description: stripHtml(e.description ?? '').slice(0, 500),
          organizer: 'Kuringa',
          location_name: e.venue?.venue ?? 'Berlin',
    ...
    for (const url of URLS) {
      try {
        ...
        const ld = extractJsonLd(html, 'kuringa-de')
        if (ld.length > 0) return ld
        const out: RawEvent[] = []
        const rx = /<(?:article|div|li)[^>]*class="[^"]*(?:event|veranstaltung|workshop|theater|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
        let m
        while ((m = rx.exec(html)) !== null) {
          ...
          out.push({
            source: 'kuringa-de', source_id: `kur-${hashStr(title)}`,
            source_url: t[1] ? new URL(t[1], url).toString() : url, title,
            description: 'Kuringa Theatre of the Oppressed event.',
            organizer: 'Kuringa', location_name: 'Berlin',
            lat: 52.5200, lng: 13.4050, starts_at: starts, cost: 'See event page',
          })
        }
