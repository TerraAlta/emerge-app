/**
 * Transition Malta — transition.org.mt
 * Maltese transition movement events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'
import { extractMultiDate, geocodeMalta } from './ch-mt-utils'
const URLS = [
  'https://www.transition.org.mt/events/',
  'https://www.transition.org.mt/',
]

export const transitionMt: SourceFetcher = {
  name: 'transition-mt',
  async fetch() {
    for (const url of URLS) {
      try {
        ...
        const jsonLd = extractJsonLd(html, 'transition-mt')
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => {
                ...
                return {
                  source: 'transition-mt', source_id: `trans-mt-${e.id}`,
                  source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                  description: stripHtml(e.description ?? '').slice(0, 500),
                  organizer: e.organizer?.[0]?.organizer ?? 'Transition Malta',
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|activity|initiative|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'transition-mt', source_id: `trans-mt-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: 'Transition event. See link for details.',
      organizer: 'Transition Malta', location_name: location,
