/**
 * Zero Waste Belgium — zerowastebelgium.be
 * Belgian zero waste community events.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://www.zerowastebelgium.be'
const URLS = [`${BASE}/evenementen/`, `${BASE}/events/`]
  ...
  async fetch() {
    for (const url of URLS) {
      try {
          ...
          if (api.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any): RawEvent => ({
                source: 'zero-waste-be', source_id: `zwb-${e.id}`,
                source_url: e.url ?? url, title: stripHtml(e.title ?? ''),
                description: stripHtml(e.description ?? '').slice(0, 500),
                organizer: e.organizer?.[0]?.organizer ?? 'Zero Waste Belgium',
        ...
        const jsonLd = extractJsonLd(html, 'zero-waste-be')
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evenement|atelier|workshop|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: 'zero-waste-be', source_id: `zwb-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: 'Zero Waste Belgium event. See link for details.',
      organizer: 'Zero Waste Belgium', location_name: 'Belgium',
