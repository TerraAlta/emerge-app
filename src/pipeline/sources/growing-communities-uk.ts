 ...
 * Hackney-based community-led food hub. Organic box schemes,
 * farmers' markets, patchwork farm, and volunteer days.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://www.growingcommunities.org'
const URLS = [`${BASE}/events`, `${BASE}/whats-on`]
const DEFAULT_LAT = 51.5074
const DEFAULT_LNG = -0.1278

export const growingCommunitiesUk: SourceFetcher = {
  name: 'growing-communities-uk',
  async fetch({ lat, lng, radiusKm }) {
    for (const url of URLS) {
        ...
        if (apiRes.ok) {
          ...
          if (data.events?.length > 0) {
            return data.events.map((e: any) => ({
              source: 'growing-communities-uk',
              source_id: `gc-${e.id}`,
              source_url: e.url ?? url,
              title: stripHtml(e.title ?? ''),
              description: stripHtml(e.description ?? '').slice(0, 500),
              organizer: e.organizer?.[0]?.organizer ?? 'Growing Communities',
              location_name: e.venue?.venue ?? e.venue?.city ?? 'Hackney, London',
      ...
      try {
        ...
        const jsonLd = extractJsonLd(html, 'growing-communities-uk')
    ...
    console.warn('[growing-communities-uk] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pattern = /<(?:article|div|li)[^>]*class="[^"]*(?:event|volunteer|harvest|market|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let match
  while ((match = pattern.exec(html)) !== null && events.length < 30) {
    ...
    events.push({
      source: 'growing-communities-uk', source_id: `gc-${hashStr(title)}`,
      source_url: tm[1] ? new URL(tm[1], BASE).toString() : baseUrl,
      title, description: 'Growing Communities event. Urban food growing, markets, and volunteer harvest days in Hackney.',
      organizer: 'Growing Communities', location_name: 'Hackney, London',
