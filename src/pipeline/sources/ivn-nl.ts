 ...
 * Netherlands' biggest nature education org, 25,000+ volunteers.
 */
        ...
        try {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length) return data.events.map((e: any) => ({
              source: SRC, source_id: `ivn-${e.id}`, source_url: e.url ?? url,
              ...
              cost: e.cost ?? 'Gratis / Zie evenement',
            }))
          }
        } catch {}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|evenement|agenda|activiteit|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `ivn-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl,
      title, description: `${ORG} activiteit. Zie link voor details.`,
