 ...
 * UK biodynamic farming and gardening. Courses, farm days,
 * workshops on Steiner-inspired ecological agriculture.
 */
import type { RawEvent, SourceFetcher } from './types'
import { stripHtml, hashStr, extractJsonLd } from './utils'

const BASE = 'https://www.biodynamic.org.uk'
const URLS = [`${BASE}/events/`, `${BASE}/whats-on/`]
const DEFAULT_LAT = 51.5074
const DEFAULT_LNG = -0.1278

export const biodynamicUk: SourceFetcher = {
  name: 'biodynamic-uk',
  async fetch({ lat, lng, radiusKm }) {
    for (const url of URLS) {
        ...
        if (apiRes.ok) {
          ...
          if (data.events?.length > 0) {
            return data.events.map((e: any) => ({
              source: 'biodynamic-uk',
              source_id: `bd-${e.id}`,
              source_url: e.url ?? url,
              title: stripHtml(e.title ?? ''),
              description: stripHtml(e.description ?? '').slice(0, 500),
              organizer: e.organizer?.[0]?.organizer ?? 'Biodynamic Association',
      ...
      try {
        ...
        const jsonLd = extractJsonLd(html, 'biodynamic-uk')
    ...
    console.warn('[biodynamic-uk] all URLs failed')
    return []
  },
}
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pattern = /<(?:article|div|li)[^>]*class="[^"]*(?:event|course|workshop|farm|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let match
  while ((match = pattern.exec(html)) !== null && events.length < 30) {
    ...
    events.push({
      source: 'biodynamic-uk', source_id: `bd-${hashStr(title)}`,
      source_url: tm[1] ? new URL(tm[1], BASE).toString() : baseUrl,
      title, description: 'Biodynamic Association event. Courses, farm visits, and workshops on biodynamic agriculture.',
      organizer: 'Biodynamic Association', location_name: 'UK',
