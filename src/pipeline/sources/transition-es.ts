/**
 * Red de Transición España — redtransicion.org/eventos
 * Spanish Transition Network events.
 */
...
const URLS = [
  'https://redtransicion.org/eventos',
  'https://www.reddetransicion.org/eventos',
]
const SRC = 'transition-es'
const ORG = 'Red de Transición España'

export const transitionEs: SourceFetcher = {
  name: SRC,
  async fetch() {
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `tr-es-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|iniciativa|actividad|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `tr-es-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento de transición. Ver enlace para detalles.', organizer: ORG,
