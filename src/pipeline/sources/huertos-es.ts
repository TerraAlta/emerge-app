/**
 * Huertos Comunitarios — redhuertas.org/eventos
 * Community garden network events in Spain.
 */
...
const URLS = [
  'https://redhuertas.org/eventos/',
  'https://huertoscomunitarios.net/eventos/',
]
const SRC = 'huertos-es'
const ORG = 'Huertos Comunitarios'
          ...
          if (apiRes.ok) {
            ...
            if (data.events?.length > 0) {
              return data.events.map((e: any) => ({
                source: SRC, source_id: `hue-es-${e.id}`, source_url: e.url ?? url,
...
function scrapeHtml(html: string, baseUrl: string): RawEvent[] {
  ...
  const pat = /<(?:article|div|li)[^>]*class="[^"]*(?:event|huerto|actividad|tribe)[^"]*"[^>]*>([\s\S]*?)<\/(?:article|div|li)>/gi
  let m
  while ((m = pat.exec(html)) !== null) {
    ...
    events.push({
      source: SRC, source_id: `hue-es-${hashStr(title)}`,
      source_url: t[1] ? new URL(t[1], baseUrl).toString() : baseUrl, title,
      description: 'Evento de huertos comunitarios. Ver enlace para detalles.', organizer: ORG,
