...
- [ ] **Land-matching in the Guild (landholders ↔ land-seekers).** Idea sparked by [Landmatch England](https://landmatchengland.org.uk/) (LWA + CSA Network + Shared Assets + Tamar Grow Local; launched Jan 2026). It's a two-sided marketplace — the hardest thing to cold-start — but it creates value at *tiny* scale (3 landowners + 10 seekers + a human broker = real matches). **Design when we build it:** concierge/human-matched, NOT a swipe app — two intake forms ("offer land" / "seek land") → private Supabase table → Pedro/coordinator matches by hand (mirrors Landmatch AND the soul doc: human judgment, no algorithms, relationship-first). **Privacy-critical:** land location + owner identity stay private, revealed only on a facilitated intro — never publicly browsable. **Cold-start plan:** seed the scarce side (landowners) by hand in ONE geography (Portugal or UK), don't spread across 23 countries; consider partnering with / surfacing Landmatch·LWA·CSA rather than cloning their network; Emerge's edge is the integrated ecosystem (learn→practice→people→land) + multi-country reach, not land alone. Nearly free to run (form + human matching), so it fits the free-Vercel move. **Revisit when:** the app has real promotion/traction — Pedro wants to grow the audience first (a marketplace needs users to fill it).
